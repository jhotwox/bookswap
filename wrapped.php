...
<body>
  ...
  <div class="container">
    <div class="row">
      <div
        ...
        id="prestados"
        style="background: linear-gradient(90deg,rgba(252, 184, 0, 0.8) 0%, rgba(255, 252, 105, 1) 20%, rgba(255, 255, 255, 1) 100%); overflow: hidden; width: 50%;"
      >
        <?php
          ...
          $cuantos_prestamos = GetValueSQL($query1, 'cuantos');

          if($cuantos_prestamos > 0) {
        ?>
          <img src="img/pdf/stars.png" class="d-none d-md-block position-absolute" width="150" style="position: absolute; top: 50px; right: 0;">
          <img src="img/pdf/star.png" class="d-none d-md-block position-absolute" width="150" style="position: absolute; top: 30px; left: 120px;">
          <img src="img/pdf/shinny.png" class="d-none d-md-block position-absolute" width="150" style="position: absolute; top: 400px; right: 0;">
          <img src="img/pdf/OIP.png" class="d-none d-md-block position-absolute" width="650" style="position: absolute; top: -10px; left: -450px; transform: rotate(270deg);">
          <?php
            $query2 = "SELECT 
              libros.id_libro,
              libros.titulo,
            ...
            $prestamo = '<div class="pt-4">
              <table class="pt-4 table-responsive w-100 mx-4">
                <thead>
                  <tr class="mb-4">
                    <th></th>
                    <th colspan="2" class="text-start" style="position: relative;">
                      <div style="
                        overflow: visible;
                        ;
                      ">
                        <h2 class="pt-4 mb-4" style="
                            color: rgba(45, 5, 200, 1);
                            left: 20%;
                          ">Libros Prestados</h2>
                      </div>  
                    </th>
              ...
              $prestamo .= '
                  <tr class="pt-8 mb-4">
                    ...
                    <td class="text-center" style="
                      ...
                      left: 10%;
                    ">
                      <h3 style="color: rgba(45, 5, 200, 1);">'.$cantidad.'</h3>
                    </td>
                  </tr>';
              $index += 1;
            }
            ...
            $prestamo = "
              ...
              '>
                <h2 class='text-center'>No se encontraron prestamos</h2>
              </div>";
          }
          echo $prestamo;
        ?>
        
        <!-- <a href="pdf/prestados.php">PDF</a>   -->
      </div>
      <div
        class="col-12 col-lg-6"
        id="leidos"
      ...
      > 
        <?php
          $query3 = "SELECT COUNT(*) AS cuantos FROM prestamos WHERE fecha_fin BETWEEN DATE_FORMAT(NOW() - INTERVAL 1 MONTH, '%Y-%m-01') AND LAST_DAY(NOW() - INTERVAL 1 MONTH) AND id_usuario_destino = $id_usuario_global";
          $cuantos_leidos = GetValueSQL($query3, 'cuantos');
          ...
          if($cuantos_leidos > 0) {
            ...
            <img src="img/pdf/star.png" class="d-none d-md-block position-absolute" width="150" style="position: absolute; top: 30px; left: 120px;">
            <img src="img/pdf/shinny.png" class="d-none d-md-block position-absolute" width="150" style="position: absolute; top: 400px; right: 0;">
            <img src="img/pdf/OIP.png" class="d-none d-md-block position-absolute" width="1280" style="position: absolute; top: -10px; left: -450px; transform: rotate(270deg);">
            <?php
            $query4 = "SELECT 
              libros.id_libro,
              libros.titulo,
            ...
            WHERE fecha_fin BETWEEN DATE_FORMAT(NOW() - INTERVAL 1 MONTH, '%Y-%m-01')
              AND LAST_DAY(NOW() - INTERVAL 1 MONTH)
              AND id_usuario_destino = $id_usuario_global
            GROUP BY libros.id_libro, libros.titulo
            ORDER BY cantidad DESC";
            $prestados = DatasetSQL($query4);
            ...
            $index = 1;
            while($row2 = mysqli_fetch_array($prestados)) {
    ...
    <div class="row p-4">
      <div class="col-6">
          ...
          ?>
          <button
            class="btn btn-danger btn-lg btn-block d-flex align-items-center justify-content-center"
            style="z-index: 10;"
            id="btnPrestamo"
          >
            <i class="fas fa-file-pdf me-4"></i> Descargar PDF
          </button>
          <?php } ?>
      </div>
      <div class="col-6">
  ...
  <script>
    ...
    document.getElementById("btnLeido").addEventListener("click", function () {
      const element = document.getElementById("leidos");
